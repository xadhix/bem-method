...
В БЭМ-методологии JavaScript используется для «оживления» веб-страницы и рассматривается как одна из [технологий реализации](../definitions/definitions.ru.md#Технология-реализации) блока. На ровне с другими технологиями (CSS, HTML, шаблоны), JavaScript может подчиняться основным правилам БЭМ-методологии.

В работе с JavaScript по БЭМ:
  * Используются единые термины блоков, элементов и модификтаоров.
  * Имена БЭМ-сущностей формируются по общим правилам именования.
  * Файлы с JavaScript-реализацией хранятся по принципам организации файловой структуры БЭМ-проекта.
...
Если блоку добавить JavaScript-функциональность (например, клик по логотипу будет вызывать дополнительное действие), файл с JavaScript-реализацией будет создан:
  * в отдельном файле;
...
* Как писать JavaScript по БЭМ
  * Общие сведения
  * Как блоки взаимодействуют друг с другом?
  * Как блок взаимодействует со своими элементами?
  * Как происходит реакция на изменения модификатора?
...
Чтобы максимально использовать возможности JavaScript-кода, написанного по БЭМ, необходимо следовать всем идеям БЭМ-методологии: единое именование, организация файловой системы, использование сборки. При соблюдении всех требований, JavaScript-реализацию блоков можно будет разделять по уровням переопределения по аналогии с CSS.

> Если реализовать все идеи БЭМ-методологии (единое именование, декларативный подход, уровни переопределния), появится возможность разделять JavaScript-реализацию блоков по уровням переопределения по аналогии с CSS.

## Как писать JavaScript по БЭМ

Разделение CSS на отдельные файлы не вызывает споров, так как компонентный подход, помимо БЭМ, реализован во многих продуктах, таких как Polymer или стандарт Web Components.

Идея разделения JavaScript-кода на отдельные файлы вызывает ряд вопросов:
* Как блоки взаимодействуют друг с другом?
* Как блок взаимодействует со своими элементами?
* Как происходит реакция на изменения модификатора?
...
JavaScript по БЭМ позволяет описывать логику работы блока как набор состояний. Под состоянием следует понимать как внешний вид (CSS, шаблоны), так и поведение. Состояния блоков выражаются через модификаторы. Изменение модификатора создает событие, которое можно использовать для работы с блоком.

#### Полиморфизм

В терминах ООП блок рассматривается как класс, а экземпляр блока — как экземпляр класса (объект).
...
В соответствии с ООП, вся функциональность блока реализуется модульно в методах класса (=блока). JavaScript-реализация одного блока отделена от другого.
...
Уровни позволяют описать новый класс на основе уже существующего с частично или полностью заимствующейся функциональностью.

#### Работа с уровнями переопределния

Как правило в БЭМ-проекте используется больше одного уровня переопределения. JavaScript, написанный по БЭМ, позволяет переопределять не только шаблоны или CSS блоков, но и JavaScript-реализацию. Каждый уровень изменяет (переопределяет или доопределяет) поведение.

Переопределение JavaScript-реализации блоков в БЭМ позволяет использовать библиотеку блоков с JavaScript-функциональностью, вносить изменения на отдельном уровне и собирать нужную реализацию со всех уровней последовательно.

Для работы с JavaScript в терминах БЭМ необходим фреймворк.

Если в проекте только один уровень переопределния, ограничения при выборе фреймворка отсутствуют. Если уровней несколько — необходим специализированный фреймворк, который удовлетворит требованиям JavaScript в БЭМ.

> В качестве примера можно рассматривать i-bem.js.
