...
Наравне с другими технологиями (CSS, HTML, шаблоны), в работе с JavaScript могут соблюдаться следующие принципы БЭМ-методологии:
  * [Единая предметная область](#Единая-предметная-область) — использование блоков, элементов и модификаторов, названных по общим [правилам именования БЭМ-сущностей](../naming-convention/naming-convention.ru.md).
  ...
  * [Использование уровней переопределения и сборки](#Работа-с-уровнями-переопределения).

## Единая предметная область

В веб-технологиях финальный продукт (например, веб-страница) состоит из разных технологий (например, HTML, CSS, JS). В БЭМ для работы во всех технологиях используются единые термины и подходы к реализации.

Так, например, JavaScript-реализация блоков не оперирует понятиями DOM-элементов, а использует следующий уровень абстракции — [БЭМ-дерево](../key-concepts/key-concepts.ru.md#БЭМ-дерево) — и работает в терминах блоков, элементов и модификаторов. В CSS модификаторы выражают внешний вид блока или его элемента. Логика работы блока или элемента в JavaScript также выражается с помощью **модификаторов** и описывается как набор состояний.

Применение единых понятий во всех технологиях реализации позволяет реализовать в JavaScript различные хелперы для работы с компонентами и отказаться от жесткого кодирования имен блоков и разделителей. Такой подход дает возможность, например, найти внутри блока все элементы с определенным именем и выставить им модификатор, проверить его значение.
...
В таком случае поиск компонента выполняется не по классу, а по блоку, который в проекте может выражаться тегом, классом, атрибутом и т.д. Отображение всплывающего окна (перевод блока `popup` в состояние `visible`) также осуществляется не по классу, а с помощью модификатора.
...
Нельзя изменять состояния в режиме runtime с помощью модификатора, напрямую меняя CSS-класс на соответствующем DOM-узле. Для корректной работы JavaScript все манипуляции с модификаторами должны производиться при помощи методов-хелперов.
...
JavaScript — это одна из технологий реализации блока, поэтому к ней могут применяться основные принципы организации и хранения кода в БЭМ-методологии:
...
> Для работы с уровнями переопределения в БЭМ можно использовать специализированный фреймворк, например, i-bem.js, так как он полностью адаптирован под требования БЭМ-методологии.
...
В простейшем случае блок соответствует DOM-узлу один к одному. DOM-узел и блок — это не всегда одно и то же. Можно разместить несколько блоков на одном DOM-узле (это называется [микс](../key-concepts/key-concepts.ru.md#Микс)), а также реализовать один блок на нескольких DOM-узлах.
...
Распространенный пример взаимодействия родительского и дочернего блоков с помощью микса элемента блока-родителя. Вложенному/дочернему блоку примиксовывается элемент родителя. Тогда для того, чтобы найти вложенный блок, родитель сначала находит свой компонент, а потом на этом же узле получает экземпляр другого блока и вызывает методы этого блока.
