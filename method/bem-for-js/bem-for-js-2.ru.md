...
Если блоку добавить JavaScript-функциональность (например, клик по логотипу будет вызывать дополнительное действие), файл с JavaScript-реализацией будет создан по тем же правилам:
  * в отдельном файле;
...
JavaScript, написанный по БЭМ, позволяет в полной мере использовать компонентный подход, реализованный в БЭМ-методологии:
  ...
  * облегчить и ускорить разработку за счет возможности разрабатывать по-блочно;
...
Перечисленные возможности опциональны и могут использоваться, исходя из нужд проекта.

Чтобы получить максимальную пользу от JavaScript, написанного по БЭМ, необходимо следовать всем идеям БЭМ-методологии: единое именование, декларативный подход, уровни переопределния. В этом случае появится возможность разделять JavaScript-реализацию блоков по уровням переопределения по аналогии с CSS.
...
JavaScript по БЭМ позволяет описывать логику работы блока в декларативном стиле — как набор состояний. Под состоянием следует понимать как внешний вид (CSS, шаблоны), так и поведение. Состояния блоков выражаются через модификаторы. Изменение модификатора создает событие, которое можно использовать для работы с блоком.
...
Как правило в БЭМ-проекте используется больше одного уровня переопределения. Почему это удобно и полезно, читай в разделе. JavaScript, написанный по БЭМ, позволяет переопределять блоки не только в шаблонах или CSS, но и в JavaScript. Каждый уровень по-своему переопределяет поведение. Таким образом можно реализовать библиотеку блоков с JavaScript-функциональностью и использовать сборку.

Если использовать один уровень переопределения в проекте, тогда ограничений в выборе фреймворка нет. Если несколько уровней — нужно научить фреймворк работать с JavaScript по БЭМ. В качестве примера можно рассматривать i-bem.js.
