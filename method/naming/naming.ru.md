...
Жесткие правила именования упрощает разработку и отладку кода. А также решает многие [насущные для веб-разработчика проблемы](problems.ru.md) только за счет соглашения по именованию CSS-селекторов.
...
* Для хранения имен блоков, элементов и модификаторов используются **CSS-классы**.

> Почему мы используем CSS-классы и какие есть альтернативы читайте [тут](faq.ru.md).

### Имя блока

Имя блока служит [неймспейсом](https://ru.wikipedia.org/wiki/Пространство_имён) для элементов и модификаторов блока.
...
Иногда к именам блоков могут добавляться различные префиксы. Причины возникновения и их значения описаны в Подробнее о том, почему сейчас
...
СОСЛАТЬСЯ на док и не вставлять
>#### Использование префиксов в именах блоков

>Иногда в имена блоков могут добавляться различные префиксы.
...
**Note** Для выражения семантической связи достаточно указать более точное имя элемента.
...
Модификаторы отделяются от имени блока или имени элемента нижним подчеркиванием (`_`).
...
для модификатора блока menu_visible
...
указан модификтор бе основного блока, а модификатор мож
