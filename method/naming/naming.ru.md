...
Для работы с БЭМ-сущностями (блоками, элементами и модификаторами) необходимо ознакомиться с правилами их именования.

Основная идея соглашения по именованию — сделать имена CSS-селекторов максимально информативными и понятными. Это поможет [упростить разработку](../faq/fixed-problems.ru.md#bem-clarify-code) и [отладку](../faq/fixed-problems.ru.md#bem-split-code) кода, а также решить некоторые [проблемы](../faq/fixed-problems.ru.md#bem-simple-code) веб-разработчиков.
...
**Важно!** В классической схеме именования модификатор не может использоваться в отрыве от своего владельца.
_________________________________________________
...
>В данном случае в записи отсутствует сам блок, на который влияет модификатор. В классической схеме модификатор не используется в отрыве от своего владельца.
...
Основное отличие данного стиля в отсутствии имени блока перед модификатором. При такой форме записи невозможно использовать [миксы](../faq/fixed-problems.ru.md#bem-mix), так как отсутствует возможность определить, к какому блоку относится модификатор.
...
Использование классического стиля имеет один существенный плюс — все предлагаемые [инструменты](https://ru.bem.info/tools/) платформы БЭМ ориентируются именно на классическое именование.
