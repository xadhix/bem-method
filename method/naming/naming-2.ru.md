...
> Почему мы используем CSS-классы и какие есть альтернативы читайте [тут]().

#### Имя блока

Имя блока формируется как `block`.

Оно служит [неймспейсом](https://ru.wikipedia.org/wiki/Пространство_имён) для его элементов и модификаторов.
...
|| HTML || CSS ||
| <code><div class="block">...</div></code> | <code>.block { color: red; }</code> |
...
</div></code> | <code>.block__elem { color: red; }</code>
**Неправильное использование CSS-правил**
<code>.block .block__elem { color: #042; }
div.block__elem { color: #042; }</code> |
...
* Модификатор типа «ключ — значение». Значение модификатора отделяется от его имени одним подчеркиванием (`_`). Например, `_size_large`, `_type_search`.
...
|| HTML || CSS ||
| <code><div class="block block_mod">...</div>
<div class="block block_theme_green block_size_large">...</div></code> | <code>.block_mod { display: none }</code> |
...
>Инструмент [bem-naming](https://ru.bem.info/tools/bem/bem-naming/#%D0%91%D0%AD%D0%9C-%D0%BD%D0%BE%D1%82%D0%B0%D1%86%D0%B8%D1%8F), который предоставляет информацию о БЭМ-сущности по строке и формирует ее строковое представление на основе БЭМ-нотации, позволяет содержит настройки классического стиля по умолчанию, но позволяет задавать и альтернативные правила.
