...
Методология БЭМ оперирует понятиями БЭМ-сущностей: [блоков](), [элементов]() и [модификаторов](). И прежде чем начать с ними работать, необходимо ознакомиться с правилами их именования.

Соглашение по именованию позволяет всегда одинаково называть БЭМ-сущности независимо от того, пишете ли вы CSS, JavaScript, шаблоны или работаете с файловой системой проекта. Благодаря общей системе вы всегда понимаете из названия, о какой БЭМ-сущности идет речь.
...
>ОФОРМИТЬ КАК ВРЕЗКУ: Существуют и другие варианты использования CSS-селекторов.
...
Имя блока формируется как `имя-блока`.
...
Имя элемента формируется также, как и имя блока — `имя-элемента`.

Для определения принадлежности элемента к блоку используется неймспейс в виде имени блока, который отделяется от имени элемента двумя подчеркиваниями.
...
**Note** Для выражения подобной связи можно просто указать более точное имя элементу:
...
Имя модификатора формируется как `имя-модификатор_значение`.
...
Например, для создания простого навигационного меню могут использоваться следующий имена классов:
...
И вполне понятные и прозрачные CSS-правила к каждому классу, например:
...
Давайте рассмотрим пример с формой поиска на сайте. Рассмотрим CSS, описывающий данную форму.

Для начала представим как бы она могла быть реализована в классическом программировании:
...
Такая форма записи не дает разработчику никаких сведений о взаимодействии селекторов: невозможно определить, к чему относятся данные селекторы. Использование глобальных переменных для создания правил в CSS делают код проекта нерасширяемым — при попытке рефакторинга многие правила будут поломаны.
...
* форма здесь уже не просто абстрактная сущность: благодаря мофификатору `form_search` можно понять, что речь идет о форме поиска.
* у этой формы есть вложенные элементы: поле `form__field` и кнопка `form__submit`.

Даже если на странице будет еще одно поле `field`, то его правила никак не будут влиять на поле, описанное в поисковой форме. Такое именование сущностей позволяет понять структуру блока на странице без подробного изучения HTML.
...
```
Пункт `snippets__item` будет иметь соответсвующее только ему CSS-правило:
...
Таким образом, элемент `item` получает уникальные независимые CSS-стили благодаря неймспейсам в виде имени блока. Теперь блоки `snippets` и `nav` можно повторно использовать на других страницах или в другом проекте.
...
Например, в проекте все ссылки изначально были синие. Существует блок `link` и прописанные ему CSS-свойства `.link { color: blue; }`. Но затем появляется необходимость сделать все ссылки в навигационном меню шапки немного больше и заметнее.
