# Соглашение об именованию

БЭМ-методология оперирует понятиями [блоков](), [элементов]() и [модификаторов]() (БЭМ-сущностями). И прежде чем начать с ними работать, необходимо ознакомиться с правилами их именования.
...
Основная идея соглашения об именовании CSS-селекторов — сделать их более понятными и научиться различать блоки, элементы и модификаторы по их именам.
...
// перефраз _ БЭМ использует CSS-классы для хранения информации о блоках, элементах и модификаторах. — Если вам не нравится, то можете использовать то -то и то-то  во врезку вместе с далее и
...
каноническая схема предполагает разделение слов дефисами.
...
Для определения принадлежности элемента к блоку, мы добавляем неймспейс в виде имени блока и отделяем его двумя подчеркиваниями.
...
**Note** для выражения подобной связи вы можете просто указать:
...
Модификаторы одтеляются от имени блока или имени элемента нижним подчеркиванием.

Модификаторы бывают булевыми (например, `_visible`) или представляют собой пару «ключ — значение» (`_size_large`, `_type_search`).

В модификаторах типа «ключ — значение» имя отделяется от значения одним подчеркиванием `_size_large`.

Значение булевого модификатора в его имени не указывается. Так имя модификатора будет записано, как `_visible`.

Полное имя модификатора создается по схеме:

* Для булевых модификаторов<br>
    `владелец_модификатор`
* Для модификаторов вида «ключ-значение»<br>
    `владелец_модификатор_значение`

##### Модификатор блока

Булевый: `модификатор`.

Вид «ключ – значение»: `блок_модификатор_значение`.
...
Булевый: `блок__элемент_модификатор`.

Вид «ключ – значение»: `блок__элемент_модификатор_значение`.
...
Вполне может оказаться, что в вашем проекте существует еще несколько вариантов использования класса `active` на других страницах, например, `.link.active`
...
Пока вы справляетесь с тем количеством данных, которые можно удержать в голове, вы можете успевать вовремя делать все необходимые изменения в коде. Но как только вы переключаетесь на другой проект или в вашем приложении добавляется еще десяток страниц, такие связи становится сложно проследить.
...
Имена CSS-классов в таком случае достаточно информативны, чтобы вам не пришлось заглядывать в HTML-код страницы. Cелектор всегда будет содержать знания о том, что данные правила влияют только на изменение конкретного блока или его элемента (в данном случае элемента `nav__item`). И вам не придется думать о `.link.active`, так как его CSS-правила будут записаны как `.link_active` и не будут зависеть от правил модификатора `active`.
...
```html
<form class="search search_theme_beautiful">
    <input class="search__field">
    <input class="search__button" type="submit">
</form>
```

Только из имен CSS-классов видно, что есть блок, названный `search`, у которого есть вложенный элемент `search__field`. Также наглядно видно, что существует модификация блока `search`, имеющая свой собственный класс `search_theme_beautiful`.
...
Если вы захотите добавить на страницу дополнительные компоненты, включающие в себя пункты, то у вас может появиться еще блок кода, содержащий класс `item`, например:
...
Изменив стили для одного компонента страницы, вы можете получить неожиданный «сюрприз» в виде сломанного зависимого компонента в другой части проекта.
...
```
И у пункта появится свое специфическое CSS-правило:
...
Можно с уверенностью сказать, что все интерфейсы абсолютно разные. Однако, также уверено можно заявлять и о том, что все интерфейсы состоят примерно из одних и тех же компонентов: все используют кнопки, поля для ввода, списки или ссылки. Именно поэтому большинство разработчиков так хотят иметь возможность использовать уже написанный код.
...
У вас есть отличный список, подходящий для вашего следующего проекта. Однако, чаще всего вы не сможете просто взять нужный вам кусок кода и вставить на новое место, так как большинство стилей сломаются из-за использования каскада. В такой форме записи стили для пунктов `.snippets.item` будут опираться на `.item`, которые тоже нужно будет перенести в новый проект, либо придется дополнять правила для переиспользуемого блока.

> Решение

Используя соглашения по именованию БЭМ, вы получаете следующие специфичные для каждого блока CSS-правила.
...
Таким образом, элемент `item` получает уникальные независимые CSS-стили благодаря неймспейсам в виде имени блока. Теперь блоки `snippets` и `nav` можно переиспользовать на других страницах или в другом проекте.
...
В нем мы используем ссылку в качестве элемента блока `nav` — `nav__link`. Но ссылка может быть и отдельным блоком на странице. **Как отличить — делать блок или элемент?**
...
При этом все общее, что есть у всех ссылок в проекте, будет описано в блоке `link`, а особенности, присущие только ссылке внутри `nav` — для `nav__link`.

Например, вы хотите, чтобы в вашем проекте все ссылки были синими, вы создаете блок `link` и прописываете ему CSS-свойства `.link { color: blue; }`. Но потом вы или ваш начальник решаете, что все ссылки в навигационном меню шапки страницы должны быть немного больше и заметнее.

В таком случае на помощь и приходят миксы: вам не нужно создавать еще один блок, вы просто добавляете специфические правила для ссылки, актуальные только навигационному меню, расположенному в шапке:
